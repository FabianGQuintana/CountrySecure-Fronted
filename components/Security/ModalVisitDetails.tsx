...
const ModalVisitDetails: React.FC<ModalVisitDetailsProps> = ({
  permission,
  guardId,
}) => {
  ...
  const handleRegisterEntry = async () => {
    if (!guardId) {
      setError(
        "No se pudo obtener el ID del guardia. Inicia sesión nuevamente."
      );
      return;
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
      <div className="bg-linear-to-br from-slate-800 to-slate-900 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[95vh] overflow-y-auto border border-purple-500/30">
        {/* Header */}
        <div className="sticky top-0 bg-linear-to-r from-purple-600 to-purple-800 text-white p-5 rounded-t-2xl flex justify-between items-center shadow-lg z-10">
          <div>
          ...
          {(!permission.resident || !permission.visitor) && (
            <div className="p-3 bg-red-500/20 border border-red-500/50 text-red-300 rounded-lg text-sm">
              <p className="font-semibold mb-1">
                ⚠️ Error: Datos obligatorios faltantes
              </p>
              <ul className="text-xs space-y-1">
                {!permission.resident && (
                  <li>• No hay información del residente</li>
                )}
                {!permission.visitor && (
                  <li>• No hay información del visitante</li>
                )}
              </ul>
            </div>
          )}
            ...
            <div className="bg-slate-700/30 rounded-lg p-4 border border-slate-600/50">
              ...
              {permission.resident ? (
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-gray-400">
                    DNI: {permission.resident.dni || "N/A"}
                  </p>
                  <p className="text-xs text-gray-400">
                    Tel: {permission.resident.phone || "N/A"}
                  </p>
                </div>
              ) : (
                <p className="text-xs text-gray-400">
          ...
          <div className="bg-slate-700/30 rounded-lg p-4 border border-slate-600/50">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <p
                  className={`text-sm font-semibold ${
                    permission.entryTime ? "text-emerald-300" : "text-gray-500"
                  }`}
                >
              ...
              >
                ...
                <p
                  className={`text-sm font-semibold ${
                    permission.departureTime ? "text-red-300" : "text-gray-500"
                  }`}
                >
          ...
          <div className="flex flex-col sm:flex-row gap-3 sticky bottom-0 bg-slate-900/95 p-4 -mx-6 -mb-6 rounded-b-2xl">
            {!permission.entryTime && permission.status === "Pending" && (
              <Button
                ...
                className="flex-1 bg-linear-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white font-bold py-3 shadow-lg hover:shadow-emerald-500/50 transition-all duration-300"
              >
            ...
            {permission.entryTime && !permission.departureTime && (
              <Button
                ...
                disabled={isProcessing}
                className="flex-1 bg-linear-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white font-bold py-3 shadow-lg hover:shadow-red-500/50 transition-all duration-300"
              >
