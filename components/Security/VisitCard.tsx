...
import { Calendar, User, Clock, CheckCircle, AlertTriangle, Package, Truck } from "lucide-react"
...
const VisitCard: React.FC<VisitCardProps> = ({ permission }) => {
  ...
  const getStatusBadge = () => {
    switch (permission.status) {
      ...
      case "Completed":
        return (
          <span className="flex items-center gap-1 text-xs px-3 py-1 bg-gray-500/20 text-gray-300 rounded-full font-medium border border-gray-500/30">
            <CheckCircle size={14} />
            Completado
          </span>
        )
 ...
 const formatDateTime = (dateString?: string) => {
  ...
  const getTypeLabel = () => {
    switch (permission.type) {
      case "Visit":
        return { label: "VISITANTE", color: "bg-purple-500/20 text-purple-300 border-purple-500/30" }
      case "Maintenance":
        return { label: "MANTENIMIENTO", color: "bg-orange-500/20 text-orange-300 border-orange-500/30" }
      case "Delivery":
        return { label: "DELIVERY", color: "bg-blue-500/20 text-blue-300 border-blue-500/30" }
      default:
        return { label: "OTRO", color: "bg-gray-500/20 text-gray-300 border-gray-500/30" }
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-5">
        <span className={`px-3 py-1 ${typeLabel.color} text-xs font-bold rounded-lg border`}>
          {typeLabel.label}
        </span>
        {getStatusBadge()}
      </div>
